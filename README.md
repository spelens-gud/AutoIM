...
如果在发送消息时遇到"无法切换到联系人的聊天窗口"错误，请使用调试工具：

```bash
# 运行联系人查找调试脚本
python debug_contact.py
```
...
1. **通过data属性查找**：支持多种data属性（data-contact-id、data-user-id等）
2. **通过文本内容查找**：遍历联系人列表，匹配文本内容
3. **通过XPath查找**：使用XPath表达式查找包含指定文本的元素
...
**问题**：登录成功后浏览器显示"Error 404 抱歉，未找到页面"

**原因**：登录成功后，1688 可能会将页面跳转到其他 URL（如用户中心），导致不在旺旺聊天页面

**解决方案**：
- **自动修复**：系统已内置自动检测和重新导航功能
  - 登录成功后，系统会自动检测当前 URL
  - 如果不在旺旺聊天页面，会自动重新导航到正确页面
  - 查看日志确认是否有"重新导航到旺旺聊天页面"的提示

- **手动处理**：
  1. 查看日志文件 `logs/wangwang_rpa.log`
  2. 确认当前页面 URL（日志中会显示）
  3. 检查配置文件中的 `wangwang.url` 是否正确
  4. 如果问题持续，尝试清除浏览器数据：
     ```bash
     rm -rf browser_data
     python main.py  # 重新登录
...
- **配置检查**：
  ...
  wangwang:
    # 确保 URL 正确
    url: https://amos.1688.com/
  ```

详细说明请查看 [docs/404_fix.md](docs/404_fix.md)
